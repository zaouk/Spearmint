...
import time
import os
from offline_data_connector import OfflineDataConnector
...
def main():
    ...
    # [KZ] Adding connector to read offline sampled data
    if 'offline-data-path' in options.keys():
        odp = options['offline-data-path']
        data_connector = OfflineDataConnector(odp)
    else:
        data_connector = None
    ...
    # Load up the chooser.
    chooser_module = importlib.import_module(
        'spearmint.choosers.' + options['chooser'])
        ...
        for resource_name, resource in resources.iteritems():
            ...
            while resource.acceptingJobs(jobs):
                ...
                suggested_job = get_suggestion(
                    chooser, resource.tasks, db, expt_dir, options,
                    resource_name, data_connector=data_connector)

                # Submit the job to the appropriate resource
                process_id = resource.attemptDispatch(
                    experiment_name, suggested_job, db_address, expt_dir)
...
def get_suggestion(chooser, task_names, db, expt_dir, options, resource_name,
    ...
    # [KZ] (These are GP hyperparameters if the chooser is GP)
...
def load_task_group(db, options, task_names=None, data_connector=None):
    ...
    if jobs:
        ...
        task_group.values = {task: np.array(
    ...
    if data_connector is not None:
        ...
        for task_key in inkeys:
            if len(task_group.values[task_key]) > 0:
                ...
                newvalues[task_key] = data_connector.values_[
                    task_key]
...
def print_diagnostics(chooser):
    ...
    trace_fh.write(
        "%d,%f,%d,%d,%d,%d\n" %
        (time.time(),
         best_val, best_job, tasks.pending.shape[0],
         tasks.invalid.shape[0],
         tasks.data.shape[0]))
    trace_fh.close()
